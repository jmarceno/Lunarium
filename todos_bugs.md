...
- Add Unique items - They should have skills or powers that are unique to them - Ex. a mace that gives you detection power of a rogue
    - Prompt:
        ...
        Now, with all these potential rules in memory, go throught the project code part responsible for combat, items, skills, monsters and
         characters, carefully analyze it and plan the best way to implement this system with the following priorities
         1. Data and new code paths for these special items, should be very well separated and organized to make it easy to add and remosve them
         2. Least amount of refactor necessary. Big refactors cause all types of bugs and unpredictable behaviour, we should keep changes to a minimum
         3. Easy of testing. We should have a way to quick add these items to the party (during development only) so we can test the items
