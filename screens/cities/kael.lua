-- Kael City Screen
-- The main navigation hub for Kael city locations
...
function kael:init()
    ...
    -- Initialize locations (same layout as Delzor for now)
    self.locations = {
        {
            name = "Tavern",
...
function kael:createUI()
    ...
    self.elements.characterInfoButton = screenManager.UI.Button(
        ...
        function() 
            ...
            gameState:changeState("characterInfo", { from = "kael" })
        end
    )
    ...
    self.elements.questLogButton = screenManager.UI.Button(
        ...
        function() 
            local gameState = require("states/gameState")
            gameState:changeState("questLog", { from = "kael" })
        end
    )
...
function kael:draw()
    ...
    local cityName = self.cityData and self.cityData.name or "Kael"
    love.graphics.print(cityName, 22, 22)
    love.graphics.setColor(1, 0.9, 0.6)
    love.graphics.print(cityName, 20, 20)
end

function kael:mousepressed(x, y, button, istouch, presses)
...
function kael:showMenu()
    self.elements.menuPanel.visible = true
end

function kael:saveGame()
    -- Prepare save data
    local saveData = {
        party = GAME.party,
        ...
        lastCity = GAME.lastCity or "Kael"
    }
...
function kael:openInventory()
    local gameState = require("states/gameState")
    gameState:changeState("inventory", { from = "kael" })
end

function kael:keypressed(key, scancode, isrepeat)
