-- Vargstad City Screen
-- The main navigation hub for Vargstad city locations
local screenManager = require("screens/screenManager")
local assetManager = require("assets/assetManager")
local saveLoad = require("utils/saveLoad")
local partyPanel = require("screens/ui_slices/partyPanel")

local vargstad = screenManager:createScreen("Vargstad")

function vargstad:init()
...
function vargstad:createUI()
    ...
    self.elements.characterInfoButton = screenManager.UI.Button(
        ...
        function() 
            ...
            gameState:changeState("characterInfo", { from = "vargstad" })
        end
    )
    ...
    self.elements.questLogButton = screenManager.UI.Button(
        ...
        function() 
            local gameState = require("states/gameState")
            gameState:changeState("questLog", { from = "vargstad" })
        end
    )
...
function vargstad:enter(params)
    ...
    if params and params.cityData then
        ...
        for _, city in ipairs(cities_definitions) do
            if city.name == "Vargstad — Where Mountains Meet the Tide." then
                self.cityData = city
                GAME.lastCity = "Vargstad — Where Mountains Meet the Tide."
    ...
    if self.cityData and self.cityData.image then
        ...
        error("City data or image path not found for Vargstad")
    end
...
function vargstad:draw()
    ...
    local cityName = self.cityData and self.cityData.name or "Vargstad"
    love.graphics.print(cityName, 22, 22)
    love.graphics.setColor(1, 0.9, 0.6)
    love.graphics.print(cityName, 20, 20)
end

function vargstad:mousepressed(x, y, button, istouch, presses)
...
function vargstad:showMenu()
    self.elements.menuPanel.visible = true
end

function vargstad:saveGame()
    -- Prepare save data
    local saveData = {
        party = GAME.party,
        ...
        lastCity = GAME.lastCity or "Vargstad"
    }
...
function vargstad:openInventory()
    local gameState = require("states/gameState")
    gameState:changeState("inventory", { from = "vargstad" })
end

function vargstad:keypressed(key, scancode, isrepeat)
