-- Sahriq City Screen
-- The main navigation hub for Sahriq city locations
local screenManager = require("screens/screenManager")
local assetManager = require("assets/assetManager")
local saveLoad = require("utils/saveLoad")
local partyPanel = require("screens/ui_slices/partyPanel")

local sahriq = screenManager:createScreen("Sahriq")

function sahriq:init()
...
function sahriq:createUI()
    ...
    self.elements.characterInfoButton = screenManager.UI.Button(
        ...
        function() 
            ...
            gameState:changeState("characterInfo", { from = "sahriq" })
        end
    )
    ...
    self.elements.questLogButton = screenManager.UI.Button(
        ...
        function() 
            local gameState = require("states/gameState")
            gameState:changeState("questLog", { from = "sahriq" })
        end
    )
...
function sahriq:enter(params)
    ...
    if params and params.cityData then
        ...
        for _, city in ipairs(cities_definitions) do
            if city.name == "Sahriq, the Sun-Worn Oasis" then
                self.cityData = city
                GAME.lastCity = "Sahriq, the Sun-Worn Oasis"
    ...
    if self.cityData and self.cityData.image then
        ...
        error("City data or image path not found for Sahriq")
    end
...
function sahriq:draw()
    ...
    local cityName = self.cityData and self.cityData.name or "Sahriq"
    love.graphics.print(cityName, 22, 22)
    love.graphics.setColor(1, 0.9, 0.6)
    love.graphics.print(cityName, 20, 20)
end

function sahriq:mousepressed(x, y, button, istouch, presses)
...
function sahriq:showMenu()
    self.elements.menuPanel.visible = true
end

function sahriq:saveGame()
    -- Prepare save data
    local saveData = {
        party = GAME.party,
        ...
        lastCity = GAME.lastCity or "Sahriq"
    }
...
function sahriq:openInventory()
    local gameState = require("states/gameState")
    gameState:changeState("inventory", { from = "sahriq" })
end

function sahriq:keypressed(key, scancode, isrepeat)
