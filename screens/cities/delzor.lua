-- Delzor City Screen
-- The main navigation hub for Delzor city locations
local screenManager = require("screens/screenManager")
local assetManager = require("assets/assetManager")
local saveLoad = require("utils/saveLoad")
local partyPanel = require("screens/ui_slices/partyPanel")

local delzor = screenManager:createScreen("Delzor")

function delzor:init()
...
function delzor:createUI()
    ...
    self.elements.characterInfoButton = screenManager.UI.Button(
        ...
        function() 
            ...
            gameState:changeState("characterInfo", { from = "delzor" })
        end
    )
    ...
    self.elements.questLogButton = screenManager.UI.Button(
        ...
        function() 
            local gameState = require("states/gameState")
            gameState:changeState("questLog", { from = "delzor" })
        end
    )
...
function delzor:enter(params)
    ...
    if self.cityData and self.cityData.image then
        ...
        error("City data or image path not found for Delzor")
    end
...
function delzor:draw()
    ...
    local cityName = self.cityData and self.cityData.name or "Delzor"
    love.graphics.print(cityName, 22, 22)
    love.graphics.setColor(1, 0.9, 0.6)
    love.graphics.print(cityName, 20, 20)
end

function delzor:mousepressed(x, y, button, istouch, presses)
...
function delzor:showMenu()
    self.elements.menuPanel.visible = true
end

function delzor:saveGame()
    -- Prepare save data
    local saveData = {
        party = GAME.party,
...
function delzor:openInventory()
    local gameState = require("states/gameState")
    gameState:changeState("inventory", { from = "delzor" })
end

function delzor:keypressed(key, scancode, isrepeat)
